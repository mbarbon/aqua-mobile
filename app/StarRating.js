...
function star (size, color, key) {
  return (
    <MaterialIcon key={Math.random()} name='grade' size={size} color={color} />
  )
}
...
export default class StarRating extends PureComponent {
  ...
  render () {
    ...
    return (
      <TouchableWithoutFeedback onPress={this._checkAndUpdateRating.bind(this)}>
        <View ref='touchableChild' style={mergedStyle}>
  ...
  _setRating (rating) {
    ...
    if (this.props.onUpdateRating) this.props.onUpdateRating(rating)
  }
