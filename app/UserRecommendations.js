...
let localRoutes = malRoutes.concat([
    { key: 'local', title: 'My list' },
]);
...
function filterRecommendations(recommendations, tagVisibility) {
    ...
    return {
        completed: filterRecommendationsArray(recommendations.completed, tagVisibility),
...
function filterLabel(index, filterState) {
    return `${filterState[index] ? "Hide" : "Show"} "${filterDefinition[index].description}"`
}
...
export default class UserRecommendations extends PureComponent {
    constructor(props) {
        ...
        this.state = {
            ...
            tagVisibility: tagVisibilityFromFilters(filterDefinition.map(e => e.initialState)),
            showSearchResults: false,
            tabState: {
                index: 0,
                routes: aquaRecommendations.isLocalUser() ?
                    localRoutes :
                    malRoutes,
            },
        };
                ...
                <View style={{ flex: 1, flexDirection: 'column' }}>
                  ...
                  <IconStrip
                    ...
                    <IconStrip.PopupButton iconName="account-circle">
                      <IconStrip.PopupItem onPress={this.changeUserMode.bind(this)}>Change user</IconStrip.PopupItem>
    ...
    renderFilterMenu() {
        return (
          <IconStrip.PopupButton iconName="filter-list">
            <IconStrip.PopupItem onPress={this.toggleFilterState.bind(this, 0)}>{filterLabel(0, this.state.filterState)}</IconStrip.PopupItem>
            <IconStrip.PopupItem onPress={this.toggleFilterState.bind(this, 1)}>{filterLabel(1, this.state.filterState)}</IconStrip.PopupItem>
            <IconStrip.PopupItem onPress={this.toggleFilterState.bind(this, 2)}>{filterLabel(2, this.state.filterState)}</IconStrip.PopupItem>
          </IconStrip.PopupButton>
        );
    }
        ...
        case 'completed':
            return (
              <AnimeList
                ...
                onRatingChanged={aquaRecommendations.isLocalUser() ?
    ...
    toggleFilterState(index) {
        this.setState((previous) => {
            ...
            return {
                ...
                filteredRecommendations: filterRecommendations(previous.recommendations, tagVisibility)
            };
        });
    }
    ...
    componentDidMount() {
        aquaRecommendations.pubSub.recommendations.subscribe(this.updateRecommendationsCallback);
        localAnimeList.pubSub.animeList.subscribe(this.recommendFromLocalListCallback);
    }

    componentWillUnmount() {
        aquaRecommendations.pubSub.recommendations.unsubscribe(this.updateRecommendationsCallback);
        localAnimeList.pubSub.animeList.unsubscribe(this.recommendFromLocalListCallback);
    }

    reloadRecommendations() {
        let userMode = aquaRecommendations.getUserMode();

        localState.getCurrentAnimeList(userMode)
    ...
    updateRecommendations(recommendations, refreshTime) {
        ...
        this.setState(previous => {
            return {
                ...
                filteredRecommendations: filterRecommendations(recommendations, previous.tagVisibility),
            };
        });
        localState.setCachedRecommendations(recommendations, this.pendingRefresh.lastRefresh)
            .catch((error) => {
                console.error(error);
            });
    }
    ...
    recommendFromLocalList(animeList) {
        let ratingList = animeList.map((item) => [item.animedbId, item.userStatus, item.userRating]);
    ...
    eventuallyRequestRecommendations(requestUserMode, ratingList) {
        ...
        if (this.state.recommendations) {
            ...
            if (startPending && now - startPending > FORCE && userMode === requestUserMode)
                return;
            let timerCallback = function() {
                ...
                if (requestUserMode === 'local')
                    ...
                    aquaRecommendations.loadMalRecommendations(ratingList);
            }.bind(this);
            if (startPending) {
