...
function showMenu(position, children) {
    if (activeMenuContexts.length !== 1)
        return;
...
class MaybeBoxShadow extends PureComponent {
    constructor(props) {
        ...
        this.clonedProps.setting.style = Object.assign({}, this.props.setting.style, {
            marginVertical: 0,
            left: 0,
            top: 0,
        });
    ...
    render() {
        if (Platform.OS == 'android') {
            ...
            return (
              <Animated.View
                style={{ ...this.props.setting.style,
                         width: setting.width,
...
class DelayedTouchableHighlight extends PureComponent {
    ...
    hideCallback() {
        if (this.pressed && this.props.onPress)
            this.props.onPress();
    }
}
        ...
        if (this.props.disabled) {
            ...
            return (
              <TouchableHighlight
                ...
                onLayout={() => this.refs.touchableHighlight.measure(this.props.onPosition)}
...
class IconStripMenuContext extends Component {
    ...
    componentDidMount() {
        ...
        this.hardwareBackPressListener = BackHandler.addEventListener('hardwareBackPress', this.closePopupOnBack.bind(this));
    }
        ...
        if (this.state.menuVisible) {
            let clickableChildren = React.Children.map(
                ...
                (child) => React.cloneElement(child, {
                    style: mergeStyle(defaultStyles.popupItemStyle, this.state.menuProps.popupItemSettings.style, child.props.style),
    ...
    makeActualPopup(clickableChildren) {
        ...
        let paddedWidth = width + 2 * radius, paddedHeight = height + 2 * radius;
        let position = this.state.position;
        let offset = this.state.menuProps.offset;

        return (
          <MaybeBoxShadow
            setting={{ width: paddedWidth,
                       height: paddedHeight,
                       border: 2,
                       radius: radius,
                       opacity: 0.2,
                       ...
                       style: { marginVertical: 5,
                                ...
                                top: position.y + offset.y - paddedHeight - this.state.keyboardHeight }}}
            >
            <View
              style={{ position: 'absolute',
...
function mergeStyle() {
    ...
    for (let i = 0; i < arguments.length; ++i) {
        ...
        if (style === null || style === undefined || isEmpty(style))
            continue;
        if (merged === null)
            merged = [];
        merged.push(style);
