...
export default class LocalState {
    resetUserMode() {
        AsyncStorage.multiRemove([malUsernameKey, userModeKey])
            .then(() => aquaRecommendations.clearMalUsername());
    }

    setLocalUser() {
        AsyncStorage.setItem(userModeKey, 'local')
            .then(() => aquaRecommendations.setLocalUser());
    }

    setMalUsernameAndLoadRecommendations(username) {
        AsyncStorage.multiSet([[malUsernameKey, username],
                               [userModeKey, 'mal']])
            .then(() => {
    ...
    loadMalRecommendations(username) {
        return aquaRecommendations.loadMalAnimeList()
            .then((animeList) => {
                // as a cache
            ...
            .then(aquaRecommendations.loadMalRecommendations.bind(aquaRecommendations));
    }

    loadUserMode(maxXyz) {
        return AsyncStorage.multiGet([userModeKey, malUsernameKey, cachedRecommendationsTimeKey])
            .then(keys => {
    ...
    _loadMalUsername() {
        return AsyncStorage.getItem(malUsernameKey)
            .then(username => {
                aquaRecommendations.setMalUsername(username);
    ...
    _loadLocalAnimeList() {
        return AsyncStorage.getItem(localAnimeListKey)
            .then((animeListString) => {
    ...
    setLocalAnimeList(animeList) {
        ...
        return AsyncStorage.setItem(localAnimeListKey, listString)
            .then(() => localAnimeList.setAnimeList(animeList));
    }
    ...
    getCurrentAnimeList(userMode) {
        if (userMode === 'mal' || userMode === 'local') {
            ...
            return AsyncStorage.getItem(key)
                .then(animeListString => JSON.parse(animeListString) || []);
